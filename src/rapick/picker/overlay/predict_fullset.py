"""Pick the whole deposition with CryoTransformer.

Derived from CryoTransformer's predict.py (MIT, Copyright (c) 2023 Jianlin Cheng,
https://github.com/jianlin-cheng/CryoTransformer, pinned at commit
a56f133f3f499562c32a6bc512eec5f115095b3b) and kept as a separate file so that a
multi-day run over a full micrograph set is not affected by the diagnostic hooks that
predict.py carries. It differs from the overlay's predict.py in exactly two ways:

  * none of the diagnostic machinery is present -- no --debug_dump, no --dump_hs, no
    --selection routes, and no forward hook on class_embed. Query selection is
...
def infer(images_path, model, postprocessors, device, output_dir):
    ...
    for img_sample in images_path:
        ...
        if extension == 'jpg':
            ...
            # Perf-only (fullset variant): vectorized grayscale->RGB channel replicate.
            # Bit-identical to the shipped putdata([(x,x,x) for x in getdata()]) loop but
            # ~100x faster (that loop iterates ~16M pixels in pure Python per micrograph).
            gray = np.asarray(orig_image.convert("L"), dtype=np.uint8)
            rgb_image = Image.fromarray(np.repeat(gray[:, :, None], 3, axis=2), "RGB")
            w, h = rgb_image.size
...
if __name__ == "__main__":
    ...
    data_path = os.path.join(resolve_test_data_root(args.data_root),
                             str(args.empiar), "images")  #the full deposition
